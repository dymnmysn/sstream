...
class Embedding(nn.Module):
    def __init__(self, num_embeddings=512, embedding_dim=512,
        ...
        super().__init__()
        assert num_embeddings % 2 == 0, "num_embeddings must be divisible by 2"
        half = num_embeddings // 2

        # Separate embedding tables for each half
        self.embeddings    = nn.Parameter(torch.randn(half, embedding_dim))
        self.embeddings_r1 = nn.Parameter(torch.randn(half, embedding_dim))        
    ...
    def decode(self, codes: torch.Tensor):
        ...
        codes_r1 = codes[:,1,:].long() - half
        decoded_main = F.embedding(codes_main, self.embeddings)
        decoded_r1 = F.embedding(codes_r1, self.embeddings_r1)
    ...
    def process_block(self, block_logits, embeddings, offset=0):
        ...
        attn_weights = F.softmax(block_logits, dim=1)  # (B, block_size, L)
        # Weighted sum
    ...
    def forward(self, logits: torch.Tensor):
        ...
        # Split logits into two parts
        main_logits = logits[:, :half, :]   # (B, half, L)
        r1_logits   = logits[:, half:, :]   # (B, half, L)

        # Process both parts separately
        main_attn, main_output, main_codes = self.process_block(main_logits, self.embeddings, offset=0)
        r1_attn,   r1_output,   r1_codes   = self.process_block(r1_logits,   self.embeddings_r1, offset=half)

        # Combine outputs
        output = main_output + r1_output

        # Save last attention/codes
        self.last_attn  = torch.cat([main_attn, r1_attn], dim=1)   # (B, N, L)
        self.last_codes = torch.stack([main_codes, r1_codes], dim=1)  # (B, 2, L)
    ...
    def redistribute_embeddings(self, reset: bool = True, strength: float = 0.05, noise_scale: float = 1e-3):
        ...
        # Example: only on first half embeddings (can be extended to both)
        mf_idx = int(torch.argmax(self.usage_counts).item())
        lf_idx = int(torch.argmin(self.usage_counts).item())

        mf = self.embeddings.data[mf_idx % (self.num_embeddings // 2)].clone()
        lf = self.embeddings.data[lf_idx % (self.num_embeddings // 2)].clone()
        self.embeddings.data[lf_idx % (self.num_embeddings // 2)] = mf + strength * lf
        ...
        if unused.numel() > 0 and used_counts.sum() > 0:
            ...
            for u, idx in zip(unused, sampled_indices):
                base = self.embeddings.data[idx % (self.num_embeddings // 2)].clone()
                noise = torch.randn(self.embedding_dim, device=self.embeddings.device) * noise_scale
                self.embeddings.data[u % (self.num_embeddings // 2)] = base + noise
    ...
    def calc_entropy(self):
        ...
        codes = self.last_codes.reshape(-1)  # flatten (B*2*L,)
        one_hot = F.one_hot(codes, num_classes=self.num_embeddings).float()  # (B*2*L, N)
    ...
    def calc_entropy_loss(self, p=0.2) -> torch.Tensor:
        ...
        soft_entropy = -(attn_weights * torch.log(attn_weights + eps)).sum(dim=1)
        soft_entropy = soft_entropy.mean()
        soft_loss = soft_weight * soft_entropy
