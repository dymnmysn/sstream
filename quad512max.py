...
class Embedding(nn.Module):
    ...
    def process_block(self, block_logits, embeddings, offset=0, k=3):
        # Get top-k indices
    ...
    def process_block(self, block_logits, embeddings, offset=0):
        # Find argmax indices (top-1 per batch)
        max_idx = torch.argmax(block_logits, dim=1, keepdim=True)  # (B, 1, L)

        # Build a one-hot mask for the max
        mask = torch.zeros_like(block_logits).scatter_(1, max_idx, 1.0)

        # Zero out all others with -inf so softmax = 1 at max, 0 elsewhere
        masked_logits = block_logits.masked_fill(mask == 0, float('-inf'))

        # Softmax → effectively one-hot at the max
        attn_weights = torch.sigmoid(masked_logits)

        # Weighted sum (only max survives)
        output = torch.einsum("bnl,nm->bml", attn_weights, embeddings)

        # Codes = max index + offset
        codes = max_idx.squeeze(1).detach() + offset
...
class StreamableModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_entropy = self.embed.calc_entropy_loss()
...
def train():
    model = StreamableModel(
        ...
        dataset='librispeech')
    trainer = pl.Trainer(
        max_epochs=150, #Bura benden: 10000 idi
        log_every_n_steps=2,
