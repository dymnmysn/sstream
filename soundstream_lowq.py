...
class Embedding(nn.Module):
    ...
    def decode(self, codes: torch.Tensor):
        """
        codes: LongTensor of shape (B, 1, L), with values in [0, num_embeddings-1]
        returns: Tensor of shape (B, M, L) where M = embedding_dim
        """
        # (B, 1, L) -> (B, L)
        codes = codes.squeeze(1).long()   # make sure it's integer indices

        # lookup embeddings (B, L, M)
    ...
    def forward(self, logits: torch.Tensor):
        ...
        # 1. Apply softened logits
        softened_logits = logits#self.t * F.relu6(logits)  # (B, N, L)

        # 2. Find top-5 indices along N
        topk_vals, topk_idx = torch.topk(softened_logits, k=5, dim=1)  # shapes: (B, 5, L), (B, 5, L)

        # 3. Create mask for top-5
        mask = torch.zeros_like(softened_logits, dtype=torch.bool)
        mask.scatter_(1, topk_idx, True)

        # 4. Zero out everything except top-5
        masked_logits = softened_logits.masked_fill(~mask, float('-inf'))  # use -inf so softmax will be zero

        # 5. Compute attention weights
        attn_weights = F.softmax(masked_logits, dim=1)  # (B, N, L), only top-5 have non-zero values

        # Weighted sum over embeddings
        output = torch.einsum("bnl,nm->bml", attn_weights, self.embeddings)  # (B, M, L)

        # Save attn_weights for entropy loss
        self.last_attn = attn_weights

        # also store hard codes
...
def train():
    model = StreamableModel(
        ...
        dataset='librispeech')
    trainer = pl.Trainer(
        max_epochs=450, #Bura benden: 10000 idi
        log_every_n_steps=2,
