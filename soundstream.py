...
class Embedding(nn.Module):
    def __init__(self, num_embeddings=512, embedding_dim=512,
                 start_t=1, end_t=10, total_epochs=100, eps=1e-8):
        super().__init__()
        self.embeddings = nn.Parameter(torch.randn(num_embeddings, embedding_dim))

        # --- NEW/CHANGED ---
        # keep embedding_dim handy
        ...
        # --- NEW/CHANGED ---
        # track usage frequency of embeddings (buffer so it moves with the module)
    ...
    def update_epoch(self, epoch: int):
        ...
        # NOTE: optional automatic redistribution per-epoch could be placed here:
        # self.redistribute_embeddings()  # <- uncomment if you want auto-redistribute

    def decode(self, codes: torch.Tensor):
        codes = codes.squeeze(1).long()
        decoded = F.embedding(codes, self.embeddings)
    ...
    def forward(self, logits: torch.Tensor):
        ...
        softened_logits = logits  # self.t * F.relu6(logits)  # (B, N, L)

        # --- CHANGED ---
        # Normal softmax over all embeddings (no top-k)
        ...
        # Save attn_weights and hard codes
        self.last_attn = attn_weights
        self.last_codes = torch.argmax(attn_weights, dim=1).detach()  # (B, L)

        # Update usage counts (accumulate across calls)
    ...
    def redistribute_embeddings(self, reset: bool = True, strength: float = 0.05, noise_scale: float = 1e-3):
        """
        Redistribute/update embeddings based on usage_counts.

        - Updates mf and lf as:
            mf <- mf + strength * lf
            lf <- mf - strength * lf
        - For embeddings with usage==0, reinitialize them based on
          proportional sampling from used embeddings according to usage frequencies.
        - By default, zeroes usage_counts after running (reset=True).
        """
        ...
        # --- Step 1: update mf and lf as before ---
        mf_idx = int(torch.argmax(self.usage_counts).item())
        lf_idx = int(torch.argmin(self.usage_counts).item())

        mf = self.embeddings.data[mf_idx].clone()
        lf = self.embeddings.data[lf_idx].clone()

        #self.embeddings.data[mf_idx] = mf + strength * lf
        self.embeddings.data[lf_idx] = mf + strength * lf

        # --- Step 2: redistribute unused embeddings proportionally ---
    ...
    def reset_usage_counts(self):
        """Explicit helper to zero usage counts without redistributing."""
        self.usage_counts.zero_()

    # existing entropy methods unchanged (copy your originals here)
    @torch.no_grad()
    def calc_entropy(self):
        """
        Calculate entropy of codebook usage (hard entropy).

        Returns:
            raw_entropy (float): Shannon entropy in nats.
        ...
        # ---------- Hard entropy ----------
        codes = self.last_codes  # (B, L)
        one_hot = F.one_hot(codes, num_classes=self.num_embeddings).float()  # (B, L, N)
        avg_counts = one_hot.mean(dim=(0, 1))  # (N,)
        avg_counts = avg_counts / (avg_counts.sum() + eps)

        # Shannon entropy (in nats)
        raw_entropy = -(avg_counts * torch.log(avg_counts + eps)).sum()

        # Normalize by maximum entropy = log(num_embeddings)
    ...
    def calc_entropy_loss(self, p=0.2) -> torch.Tensor:
        ...
        avg_loss = avg_weight * (max_entropy - avg_entropy)  # maximize

        return soft_loss + avg_loss
...
class StreamableModel(pl.LightningModule):
    def __init__(
        ...
        self.embed = Embedding(num_embeddings=512, embedding_dim=512, 
    ...
    def debugencode(self,input):
        x = self.encoder(input)
        y = self.embed(x)
        codes = torch.argmax(self.embed.last_attn,dim=1)
    ...
    def encode(self, input):
        x = self.encoder(input)
        x = self.embed(x)
        codes = torch.argmax(self.embed.last_attn,dim=1,keepdim=True)
        return codes

    def decode(self,codes):
        x = self.embed.decode(codes)
        x = self.decoder(x)
        return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_entropy = 100*self.embed.calc_entropy_loss()

        g_feat_loss = g_feat_loss / 3
        g_adv_loss = (g_stft_loss + g_wave_loss) / 4
        g_loss = g_adv_loss + 100 * g_feat_loss + g_rec_loss #+ loss_entropy
        self.log("g_loss", g_loss, prog_bar=True)
        self.log("codes_entropy", codes_entropy, prog_bar=True)
        self.log("loss_entropy", loss_entropy, prog_bar=True)
...
def train():
    model = StreamableModel(
        batch_size=512,
        sample_rate=16_000,
        segment_length=15950, #Bura benden 32270 idi
        padding='same',
        dataset='librispeech')
    trainer = pl.Trainer(
        max_epochs=350, #Bura benden: 10000 idi
        log_every_n_steps=2,
        ...
        callbacks=[
            pl.callbacks.ModelCheckpoint(save_last=True, every_n_train_steps=1500) #Bura benden 50000 idi
        ],
    )
    trainer.fit(
        model,
