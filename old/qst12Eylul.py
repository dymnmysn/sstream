...
class ResNet2d(nn.Module):
    ...
    def forward(self, input):
        ...
        # ReflectionPad2d can fail on very short feature maps after multiple
        # downsampling stages. Use the original reflection padding when possible,
        # but switch to constant padding for tiny maps so training remains stable.
        pad = self.pad.padding
        try:
            x = self.pad(x)
        except RuntimeError:
...
class STFTDiscriminator(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # torch.stft uses reflect padding internally, which is invalid when the
        # signal is shorter than n_fft. Pad with zeros first so the STFT remains
        # valid for short segments used during training.
        if x.shape[-1] < self.n_fft:
            pad_total = self.n_fft - x.shape[-1]
            left = pad_total // 2
...
def train():
    ...
    # --- Trainer setup ---
    trainer = pl.Trainer(
        max_epochs=1200,  # Bura benden: 10000 idi
        log_every_n_steps=2,
        devices=1,  # Bura benden
        precision='16-mixed',
